...
pair<int,int>* maximumGeneratingForest(vector< pair<int,int> >* adjacency_list,int num_vertices,int* num_edges_generated)
{
			...
			for(int vertex=0;vertex<num_vertices;vertex++)
			{
				if(conex_component[vertex]==k)
				{
					...
					for(int j=0;j<adjacency_list[vertex].size();j++)
					{
						...
						if(edge_weight>max_weight && conex_component[vertex_checked]!=conex_component[vertex])
						{
